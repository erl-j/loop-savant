...
class CLModel {
        ...
        for (let pitch = 0; pitch < MODEL_PITCHES; pitch++) {
            ...
            for(let t = 0; t < MODEL_TIMESTEPS; t++){
                if (flat_roll[pitch * MODEL_TIMESTEPS + t] == 1 && !note_on) {
                    ...
                    note_sequence.push({ pitch: pitch, onset: onset* 2 , duration: duration * 2 });
                }
            }
        }
    ...
    async generate(x_in, mask_in, n_steps, temperature, activityBias) {
        ...
        maskRectangles = maskRectangles.map(rectangle => ({...rectangle, startPitch: fullScale[rectangle.startRow], endPitch: fullScale[rectangle.endRow], startTimestep: rectangle.startCol*2, endTimestep: rectangle.endCol*2}));

        let n_notes = 32;

        // generate superpositions for each rectangle
        let rectangleSuperpositions = [];
        let priorSuperposition = this.prepare_superposition([...MAJOR_SCALE,-1], [..._.range(0, CLM_N_DURATIONS, 2),-1], [..._.range(0, CLM_N_DURATIONS, 2),-1],null);
        for (let i = 0; i < maskRectangles.length; i++) {
            let rectangle = maskRectangles[i];
            let rectangleSuperposition = this.prepare_superposition([..._.range(rectangle.startPitch, rectangle.endPitch + 1),-1], [..._.range(rectangle.startTimestep,rectangle.endTimestep + 1),-1],[..._.range(2,64,2),-1], null);
        ...
        console.log("combinedSuperposition", combinedSuperposition);
        ...
        console.log("note_sequence", note_sequence);
